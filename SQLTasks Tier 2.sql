...
SELECT * 
FROM (SELECT f.name, CONCAT( m.firstname, ' ', m.surname ) AS member,
CASE WHEN b.memid =0
THEN b.slots * f.guestcost
ELSE b.slots * f.membercost
END AS cost
...
SELECT CONCAT( m1.surname, ',', m1.firstname ) AS member, CONCAT( m2.surname, ',', m2.firstname ) AS recommend_by_name
FROM Members AS m2
...
SELECT name AS facility, EXTRACT(MONTH FROM starttime) AS MONTH, COUNT( memid ) AS monthly_usage
FROM Bookings AS b
INNER JOIN Facilities AS f ON b.facid = f.facid
WHERE b.memid !=0
